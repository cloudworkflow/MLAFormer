...
class TransformerModel(nn.Module):
    ...
    def forward(self, series_id: int, x: torch.Tensor):
        ...
        if self.seq_num:
            embedding_sum = torch.zeros(batch_size, length).type(torch.LongTensor).to(self.device)
        ...
        x=x.to(self.device)
        embedding_sum=embedding_sum.to(self.device)   #[32,96,512]
        x = torch.cat((x, embedding_sum), dim=2).to(self.device)  #[32,96,519]
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        # n_time_series, n_head, sub_len, num_layer, n_embd,
        # forecast_history: int, dropout: float, scale_att, q_len, additional_params: Dict, seq_num=None
        self.transformer = TransformerModel(args.n_time_series, args.n_head,args.sub_len,
                                            args.num_layer, args.n_embd, args.seq_len,
                                            args.dropout,  False, args.q_len, args.additional_params,
                                            seq_num=args.pred_len)

        self.quantiles=args.quantiles
        self.quan=args.quan
        self.softplus = nn.Softplus()
        self.mu = torch.nn.Linear(args.n_time_series + args.n_embd, args.n_time_series, bias=True).cuda()
        self.sigma = torch.nn.Linear(args.n_time_series + args.n_embd,args.n_time_series*len(self.quantiles) if self.quan else args.n_time_series , bias=True).cuda()
        self._initialize_weights()
        self.mu_mode = None
        self.forecast_len_layer = None
        if args.pred_len:
            self.forecast_len_layer = nn.Linear(args.seq_len, args.pred_len).cuda()
    ...
    def forward(self, x: torch.Tensor, series_id: int = None):
        ...
        if self.quan:
            sigma=sigma.view(sigma.size(0), sigma.size(1),-1, len(self.quantiles))
        return sigma
