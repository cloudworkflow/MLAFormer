...
from layers.AutoCorrelation import AutoCorrelation, AutoCorrelationLayer,AutoCorrelation_loss, AutoCorrelation_loss_V
from layers.Autoformer_EncDec import Encoder, Decoder, EncoderLayer, DecoderLayer, my_Layernorm, series_decomp
import math
import numpy as np
from models.encoder import ConvLayer,EncoderStack
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Encoder
        inp_lens=list(range(len(configs.s_layers)))
        encoders=[
            Encoder(
            [
                EncoderLayer(
                ...
                DecoderLayer(
                    ...
                    AutoCorrelationLayer(
                        AutoCorrelation(False, configs.factor, attention_dropout=configs.dropout,
                                        output_attention=False),
                        configs.d_model, configs.n_heads,configs.q_len),
                    configs.d_model,
                    configs.c_out,
                    configs.d_ff,
                    moving_avg=configs.moving_avg,
