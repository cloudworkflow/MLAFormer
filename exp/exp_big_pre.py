from data.data_loader import Dataset_ETT_hour_vlength, Dataset_ETT_minute_missing, Dataset_Custom_missing, Dataset_Custom_bus, Dataset_Pred, Dataset_Custom_syn
from data.data_loader import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom, Dataset_big
...
from utils.ChildTuningOptimizer import ChildTuningAdamW

class Exp_big_pre(Exp_Basic):
    def __init__(self, args):
        super(Exp_big_pre, self).__init__(args)
    ...
    def _build_model(self):
        ...
        if self.args.model=='informer' or self.args.model=='informerstack' or self.args.model=='logsparse' or  self.args.model=='Transformer' or self.args.model=="Autoformer" or self.args.model=="Autoformerstack" or self.args.model=="ts2vec" or self.args.model=="ts2vec_former" or self.args.model=="TFT" :
    ...
    def _get_data(self, flag):
        args=self.args
        data_dict={
            'ETTh1': Dataset_ETT_hour,#Dataset_ETT_hour,
    ...
    def _get_data_pre(self, flag):
        args=self.args
        data_dict={
            'ETTh1': Dataset_big,#Dataset_ETT_hour,
            'ETTh2': Dataset_big,
            'ETTm1': Dataset_big,
            'ETTm2': Dataset_big,
            'WTH': Dataset_Custom,
    ...
    def train(self, setting):
        ...
        self.model.pred_len=0
        train_data, train_loader = self._get_data_pre(flag = 'train')
        vali_data, vali_loader = self._get_data_pre(flag = 'val')
        # test_data, test_loader = self._get_data(flag = 'test')

        path = os.path.join('./checkpoints_big_pre/')
        ...
        for epoch in range(5):#self.args.train_epochs):
            iter_count = 0
            train_loss = []
            self.model.train()
            ...
            # test_loss = self.vali_pre(test_data, test_loader, criterion)
        ...
        best_model_path = path+'/model.pth.tar'

        saved_dict={
            'model': self.model.state_dict(),
            'opt': model_optim.state_dict()
        }
    ...
    def freeze(self):
        for layer in self.model.modules():
            if isinstance(layer,nn.LayerNorm):
                layer.eval()
    def train_full(self,train_data,train_loader,model_optim,criterion,train_loss,epoch,train_steps,iter_count,time_now):
        self.model.train()
        # self.freeze()
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
            iter_count+=1
    ...
    def freeze_by_names(self,model, layer_names):
        self.set_freeze_by_names(model, layer_names, True)
    def unfreeze_by_names(self, model, layer_names):
        self.set_freeze_by_names(model, layer_names, False)

    def test(self, setting):
        best_model_path=os.path.join('./checkpoints_big_pre/','model.pth.tar')#+'/model.pth.tar'#self.args.data
        if not os.path.exists(best_model_path):
            self.train(setting)
        self.args.pred_len=self.args.pred_len_full
        ...
        # for para in list(self.model.parameters()):
        #     para.requires_grad=False
        for para in list(self.model.parameters())[:-5]:#[:-5g] [:-1] [:-10] [:5]
            para.requires_grad=False
        for para in list(self.model.parameters())[-3:]:#[:-5g] [:-1] [:-10] [:5]
            para.requires_grad=False
        # self.unfreeze_by_names(self.model, ('con'))#en-dec con
        # for name, param in self.model.named_parameters():  # 查看可优化的参数有哪些
        #     if 'conv' in name and 'enc' in name or ('conv' in name and 'decoder_1' in name): #out_projection
        #         print(name)
        #         continue
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        if not os.path.exists(path+'/checkpoint.pth'):
            time_now=time.time()
            train_steps=len(train_loader)
            early_stopping=EarlyStopping(patience=self.args.patience, verbose=True)
            # model_optim=self._select_optimizer()
            model_optim=optim.Adam(filter(lambda p: p.requires_grad, self.model.parameters()), lr=0.001)#全0.001 0.34 0.0001 不好 0.001g 0.32 0.0001 e3 0.32 0.00001b
            # model_optim.load_state_dict(checkpoint['opt'])
            #
            ...
            #     "eps": 1e-6,
            # }
            # model_optim=ChildTuningAdamW(optimizer_grouped_parameters, reserve_p=1.0, mode='ChildTuning-F', **optimizer_kwargs)
            ...
            for epoch in range(self.args.train_epochs):
                ...
                train_loss=self.train_full(train_data,train_loader,model_optim,criterion,train_loss,epoch,train_steps,iter_count,time_now)
